...
fof(lemma_7, lemma, ! [X] : product(additive_identity,h(additive_identity,X),X), inference(instantiate, [status(thm)], [f18])).
fof(s1, plain, product(additive_identity,h(additive_identity,b),b), inference(instantiate, [status(thm)], [lemma_7])).
fof(s2, plain, product(additive_identity,h(additive_identity,b),b), inference(instantiate, [status(thm)], [lemma_7])).
fof(s3, plain, sum(additive_identity,additive_identity,additive_identity), inference(instantiate, [status(thm)], [f2])).
fof(s4, plain, product(additive_identity,h(additive_identity,b),b), inference(instantiate, [status(thm)], [lemma_7])).
fof(lemma_8, lemma, sum(b,b,b), inference(mp, [status(thm)], [f14, s1, s2, s3, s4])).
...
fof(lemma_9, lemma, sum(b,additive_inverse(b),b), inference(mp, [status(thm)], [f8, s5, lemma_8, s6])).
fof(s7, plain, sum(b,additive_inverse(b),additive_identity), inference(instantiate, [status(thm)], [f6])).
fof(goal_1, theorem, b = additive_identity, inference(mp, [status(thm)], [f16, s7, lemma_9])).
