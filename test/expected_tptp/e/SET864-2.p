...
fof(lemma_6, lemma, c_lessequals(c_Union(v_c,t_a),v_x(c_Union(v_c,t_a)),tc_set(t_a)), inference(mp, [status(thm)], [cls_conjecture_4, cls_conjecture_2])).
fof(goal_1, theorem, c_in(c_Union(v_c,t_a),v_S,tc_set(t_a)), inference(instantiate, [status(thm)], [cls_conjecture_2])).
fof(s1, plain, c_in(v_x(c_Union(v_c,t_a)),v_S,tc_set(t_a)), inference(mp, [status(thm)], [cls_conjecture_3, cls_conjecture_2])).
fof(goal_2, theorem, c_Union(v_c,t_a) = v_x(c_Union(v_c,t_a)), inference(mp, [status(thm)], [cls_Zorn_Omaxchain__Zorn_0, s1, cls_conjecture_0, lemma_6])).
