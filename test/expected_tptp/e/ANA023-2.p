...
fof(lemma_12, lemma, ! [X] : c_plus(c_0,X,t_b) = X, inference(mp, [status(thm)], [cls_OrderedGroup_Ocomm__monoid__add__class_Oaxioms_0, s1])).
fof(s2, plain, class_OrderedGroup_Opordered__ab__group__add(t_b), inference(mp, [status(thm)], [clsrel_Ring__and__Field_Oordered__idom_54, tfree_tcs])).
fof(s3, plain, c_lessequals(c_plus(c_0,v_g(v_x),t_b),v_k(v_x),t_b), inference(mp, [status(thm)], [cls_OrderedGroup_Ocompare__rls__9_0, s2, cls_conjecture_1])).
fof(lemma_13, lemma, c_lessequals(v_g(v_x),v_k(v_x),t_b), inference(rewrite, [status(thm)], [lemma_12, s3])).
fof(s4, plain, class_Orderings_Olinorder(t_b), inference(mp, [status(thm)], [clsrel_Ring__and__Field_Oordered__idom_33, tfree_tcs])).
fof(s5, plain, class_Orderings_Oorder(t_b), inference(mp, [status(thm)], [clsrel_Orderings_Olinorder_4, s4])).
fof(s6, plain, c_lessequals(v_g(v_x),v_f(v_x),t_b), inference(mp, [status(thm)], [cls_Orderings_Oorder__class_Oorder__trans_0, s5, cls_conjecture_2, lemma_13])).
fof(lemma_14, lemma, c_lessequals(c_plus(c_0,v_g(v_x),t_b),v_f(v_x),t_b), inference(rewrite, [status(thm)], [lemma_12, s6])).
fof(s7, plain, class_OrderedGroup_Opordered__ab__group__add(t_b), inference(mp, [status(thm)], [clsrel_Ring__and__Field_Oordered__idom_54, tfree_tcs])).
fof(goal_1, theorem, c_lessequals(c_0,c_minus(v_f(v_x),v_g(v_x),t_b),t_b), inference(mp, [status(thm)], [cls_OrderedGroup_Ocompare__rls__9_1, s7, lemma_14])).
