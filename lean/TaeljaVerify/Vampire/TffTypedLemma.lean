...
namespace VampireTffTypedLemma
...
axiom ax4 : s a
-- Axiom 5
axiom ax5 : ∀ (x : α) (y : α), s x → q x → y = (home x) → p x y
...
-- Goal 1
theorem taelja_goal1 : p a (home a) := by
  ...
  have h3 : p a (home a) := by first | (exact ax5 _ _ h1 h2) | (apply ax5 <;> (first | assumption | rfl | exact Eq.symm (by assumption) | exact Eq.trans (by assumption) (by assumption) | exact Eq.trans (Eq.symm (by assumption)) (by assumption) | exact Eq.trans (by assumption) (Eq.symm (by assumption)) | exact Eq.trans (Eq.symm (by assumption)) (Eq.symm (by assumption)) | exact taelja_elem))
  exact h3

end VampireTffTypedLemma
