...
theorem taelja_lemma14 : ∀ (x : α), (c_plus c_0 x t_b) = x := by
  ...
  have h2 : (c_plus c_0 x t_b) = x := by apply ax8 <;> (first | assumption | exact Eq.symm (by assumption))
  exact h2
...
theorem taelja_lemma16 : c_lessequals (c_plus c_0 (v_g v_x) t_b) (v_f v_x) t_b := by
  have h1 : class_Ring__and__Field_Oordered__idom t_b := by apply ax1
  have h2 : class_Orderings_Olinorder t_b := by apply ax4 <;> (first | assumption | exact Eq.symm (by assumption))
  have h3 : class_Orderings_Oorder t_b := by apply ax5 <;> (first | assumption | exact Eq.symm (by assumption))
...
theorem taelja_goal1 : c_lessequals c_0 (c_minus (v_f v_x) (v_g v_x) t_b) t_b := by
  ...
  have h3 : c_lessequals c_0 (c_minus (v_f v_x) (v_g v_x) t_b) t_b := by apply ax11 <;> (first | assumption | exact Eq.symm (by assumption))
  exact h3
